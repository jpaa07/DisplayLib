...
public:
  ...
  void Run() {
    ...
    // Diagonals.
    DrawLine(canvas(), 10, 0, 10, height, Color(0, 0, 255));
...
public:
  ...
  ImageScroller(RGBMatrix *m, int scroll_jumps, int scroll_ms = 20)
...
class MyName : public ThreadedCanvasManipulator {
public:
  MyName(Canvas *m, int delay_ms = 200) : ThreadedCanvasManipulator(m), delay_ms_(delay_ms){}
  void Run() {
    const int width = canvas()->width()-1;
    const int height = canvas()->height()-1;
   
    //Drawing "J"
    DrawLine(canvas(),width/3,2,width/3,height-2,Color(255, 0, 0));
    DrawLine(canvas(),width/3,height-2,(width/3)-19,height-2,Color(255, 0, 0));
    DrawLine(canvas(),(width/3)-19,height-2,(width/3)-19,height-8,Color(255, 0, 0));

    //Drawing "P"
    DrawLine(canvas(),(width/3)+20,2,(width/3)+20,height-2,Color(255, 0, 0));
    DrawLine(canvas(),(width/3)+20,2,(width/3)+40,2,Color(255, 0, 0));
    DrawLine(canvas(),(width/3)+40,2,(width/3)+40,12,Color(255, 0, 0));
    DrawLine(canvas(),(width/3)+20,12,(width/3)+40,12,Color(255, 0, 0));
  }
    int delay_ms_;
};
...
public:
  ...
  const char *text = "Juan Pablo Arango";
  rgb_matrix::Font font;
  font.LoadFont(bdf_font_file);
  DrawText(canvas(),font, 0, 8 + font.baseline(), color,text);
  }
};
...
int main(int argc, char *argv[]) {
 ...
 case 12:
   image_gen = new MyName(canvas,scroll_ms);
